...
@section('title', 'Create Inter-School Event')

@section('content')
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Create Inter-School Event</h1>
        <p class="mt-1 text-sm text-gray-600">Create a new event that schools can participate in</p>
    </div>

    <div class="bg-white shadow rounded-lg p-6">
        <form action="{{ route('inter-school-events.store') }}" method="POST">
            @csrf
            ...
            <div class="mb-6">
                ...
                <select name="event_category" id="event_category"
                    ...
                    <option value="Cultural" {{ old('event_category') == 'Cultural' ? 'selected' : '' }}>Cultural</option>
                    <option value="Academic" {{ old('event_category') == 'Academic' ? 'selected' : '' }}>Academic</option>
                    <option value="Science" {{ old('event_category') == 'Science' ? 'selected' : '' }}>Science</option>
                    <option value="Technology" {{ old('event_category') == 'Technology' ? 'selected' : '' }}>Technology</option>
                    <option value="Arts" {{ old('event_category') == 'Arts' ? 'selected' : '' }}>Arts</option>
            ...
            <div class="mb-6">
                ...
                <select name="status" id="status" required
                    ...
                    <option value="draft" {{ old('status') == 'draft' ? 'selected' : '' }}>Draft (Save without inviting schools)</option>
                    <option value="published" {{ old('status') == 'published' ? 'selected' : '' }}>Published (Invite all schools immediately)</option>
                </select>
                @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-gray-500">You can publish draft events later</p>
            </div>
