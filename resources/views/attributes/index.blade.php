...
<div class="max-w-10xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Global Attributes</h1>
        <a href="{{ route('attributes.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700">
            Add Attribute
        </a>
    </div>
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm font-medium text-gray-900">
                            <a href="{{ route('attributes.show', $attribute) }}" class="hover:text-blue-600">{{ $attribute->name }}</a>
                        </div>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <a href="{{ route('attributes.show', $attribute) }}" class="text-indigo-600 hover:text-indigo-900 mr-3">View</a>
                        <a href="{{ route('attributes.edit', $attribute) }}" class="text-blue-600 hover:text-blue-900 mr-3">Edit</a>
                        <form action="{{ route('attributes.toggle-status', $attribute) }}" method="POST" class="inline">
                            @csrf
                        ...
                        <form action="{{ route('attributes.destroy', $attribute) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this attribute?')">
                            @csrf
                            @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500">No global attributes found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
