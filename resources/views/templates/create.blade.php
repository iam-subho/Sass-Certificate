...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-6">
        <h1 class="text-3xl font-bold text-gray-900">Create Certificate Template</h1>
    </div>
        ...
        <form action="{{ route('templates.store') }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <textarea name="html_content" id="html_content" rows="20" required
                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 font-mono text-sm">{{ old('html_content') }}</textarea>
                @error('html_content')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
